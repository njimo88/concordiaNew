...
class ProfessionnelsController extends Controller
{
    ...
    public function  editPro(Request $request, $user_id)
    {
        ...
        $validatedData = $request->validate([
            ...
            'Lundi' => 'regex:/^\d+(\.\d{1,2})?$/|max:191',
            'Mardi' => 'regex:/^\d+(\.\d{1,2})?$/|max:191',
            'Mercredi' => 'regex:/^\d+(\.\d{1,2})?$/|max:191',
            'Jeudi' => 'regex:/^\d+(\.\d{1,2})?$/|max:191',
            'Vendredi' => 'regex:/^\d+(\.\d{1,2})?$/|max:191',
            'Samedi' => 'regex:/^\d+(\.\d{1,2})?$/|max:191',
            'Dimanche' => 'regex:/^\d+(\.\d{1,2})?$/|max:191',
            'Salaire' => 'regex:/^\d*(\.\d{1,2})?$/|max:191',
            'Prime' => 'regex:/^\d*(\.\d{1,2})?$/|max:191',
