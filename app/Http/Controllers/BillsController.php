    ...
    public function getData(Request $request)
    {
        if ($request->has('statusOldBills')) {
            $query = DB::table('old_bills')
                ...
                ->leftJoin('warnings', 'users.family_id', '=', 'warnings.family_id')
                ->select('old_bills.*', 'old_bills.status as bill_status', 'bills_payment_method.icon', 'users.name', 'users.lastname', 'bills_payment_method.payment_method as payment_method_alias', 'bills_payment_method.image', 'bills_status.status as status_alias', 'bills_status.image_status', 'bills_status.row_color', 'warnings.level')
            ...
            $query = DB::table('bills')
                ...
                ->leftJoin('warnings', 'users.family_id', '=', 'warnings.family_id')
                ->select('bills.*', 'bills.status as bill_status', 'bills_payment_method.icon', 'users.name', 'users.lastname', 'bills_payment_method.payment_method as payment_method_alias', 'bills_payment_method.image', 'bills_status.status as status_alias', 'bills_status.image_status', 'bills_status.row_color', 'warnings.level')
        ...
        $data = DataTables::of($query)
            ...
            ->addColumn('warning', function ($row) {
                return $row->level;
            })
    ...
    */
    ...
    public function showBill($id)
    {
        ...
        $billsQuery = DB::table('bills')
            ...
            ->leftJoin('warnings', 'users.family_id', '=', 'warnings.family_id')
            ->where('bills.id', $id)
            ->select('bills.*', 'bills_status.row_color', 'bills_status.status as bill_status', 'users.name', 'users.lastname', 'users.email', 'users.family_id', 'users.phone', 'users.address', 'users.city', 'users.zip', 'users.country', 'users.birthdate', 'bills_payment_method.payment_method as method', 'warnings.level');
        ...
        $oldBillsQuery = DB::table('old_bills')
            ...
            ->select('old_bills.*', 'bills_status.row_color', 'bills_status.status as bill_status', 'users.name', 'users.lastname', 'users.email', 'users.family_id', 'users.phone', 'users.address', 'users.city', 'users.zip', 'users.country', 'users.birthdate', 'bills_payment_method.payment_method as method', 'warnings.level');
    ...
    public function updateWarningLevel(Request $request)
    {
        try {
            $family_id = $request->input('family_id');
            $warningLevel = $request->input('level');

            if ($warningLevel == 0) {
                Warning::where('family_id', '=', $family_id)->delete();
            } else {
