...
<body>
   ...
   <div class="modal fade" id="settings" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <header id="header" class="header fixed-top d-flex align-items-center">
        <div class="d-flex align-items-center justify-content-between"> <a href="{{ route('admin.index') }}" class="logo d-flex align-items-center"> <img src="{{ asset('assets/images/LogoHB.png') }}" alt=""> <span class="d-none d-lg-block">Admin</span> </a> <i class="bi bi-list toggle-sidebar-btn"></i></div>
            ...
            @if ( auth()->user()->roles->estAutoriserDeVoirCours || auth()->user()->roles->estAutoriserDeVoirAnimations || auth()->user()->roles->estAutoriserDeVoirStatsExports || auth()->user()->roles->estAutoriserDeVoirValiderCertificats)
               <li class="nav-item">
                  ...
                  <ul id="club-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
                     ...
                     @if (auth()->user()->roles->estAutoriserDeVoirValiderCertificats)<li> <a href="{{ route('validationCertificats') }}"><span style="color: #f59f00; margin-right:10px" class="fa fa-stamp fa-fw mr-1"></span><span>Valider Certificats @php $certificatsEnAttente = \App\Models\MedicalCertificates::where('validated', 0)->count(); @endphp @if($certificatsEnAttente > 0) <span class="badge bg-danger" style="margin-left: 5px;">{{ $certificatsEnAttente }}</span> @endif</span> </a></li>@endif
