...
@section('content')
    <main id="main" class="main">
        ...
        <div class="modal fade" id="declarationList" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div style="max-height: 600px !important;" class="modal-body overflow-auto"
                        id="declarationListContainer">
                        <!-- content -->
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                                    ...
                                    @foreach ($pro as $pros)
                                        <tr>
                                            ...
                                            @if (auth()->user()->roles->edit_ajout_professionnel)
                                                <td><button data-toggle="modal" data-target="#editPro{{ $pros->cle }}"
                                                        type="button" class="btn btn-warning">Modifier</button>
                                                    @include('admin.modals.editPro')</td>
                                            @endif
                                            @if (auth()->user()->roles->declarer_heure_professionnel)
                                                <td><a type="button" class="btn btn-danger"
                                                        href="{{ route('Professionnels.declarationHeures', $pros->id_user) }}">Déclarer
                                                        les heures</a></td>
                                            @endif
                                            @if (auth()->user()->roles->voir_declaration_professionnel)
                                                <td><button data-user-id="{{ $pros->id_user }}" type="button"
                                                        class="declarationList btn btn-primary">Anciennes
                                                        déclarations</button></td>
                                            @endif
                                        </tr>
                                    @endforeach
                ...
                @if (auth()->user()->roles->edit_ajout_professionnel)
                    <div style="display: flex; justify-content: flex-start; " class="col-lg-12 float-right">
                        <button data-toggle="modal" data-target="#addPro" type="button" class="mt-4 btn btn-dark">Ajouter
                            un professionnel<i style="font-weight:900;" class="m-2 fa-regular fa-circle-plus"></i></button>
