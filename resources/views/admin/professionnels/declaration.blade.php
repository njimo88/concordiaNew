...
//2021-2022
    $toussaint[2021]   = array (mktime(0, 0, 0, 10,  17,  2021) , mktime(0, 0, 0, 10,  30,  2021)); //ok
    $noel[2021] = array (mktime(0, 0, 0, 12,  19,  2021) , mktime(0, 0, 0, 1,  2,  2022)); //ok
    $hiver[2022]   = array (mktime(0, 0, 0, 2,  20,  2022) , mktime(0, 0, 0, 3,  6,  2022)); //ok
    $printemps[2022] = array (mktime(0, 0, 0, 4,  24,  2022) , mktime(0, 0, 0, 5,  9,  2022)) ; //ok
    $ete[2022]  = array (mktime(0, 0, 0, 7,  6,  2022) , mktime(0, 0, 0, 8,  31,  2022)); //ok
//2022-2023
    $toussaint[2022]   = array (mktime(0, 0, 0, 10,  16,  2022) , mktime(0, 0, 0, 10,  29,  2022)); //ok
    $noel[2022] = array (mktime(0, 0, 0, 12,  18,  2022) , mktime(0, 0, 0, 1,  1,  2023)); //ok
    $hiver[2023]   = array (mktime(0, 0, 0, 2,  19,  2023) , mktime(0, 0, 0, 3,  5,  2023)); //ok
    $printemps[2023] = array (mktime(0, 0, 0, 4,  23,  2023) , mktime(0, 0, 0, 5,  8,  2023)) ; //ok
    $ete[2023]  = array (mktime(0, 0, 0, 7,  5,  2023) , mktime(0, 0, 0, 8,  31,  2023)); //ok
//2023-2024
...
// User not found, handle error accordingly
		...
		if (file_exists ($fichier_demande_csv))
		{
			...
			$demande_on_1 = ' hidden="hidden" ' ;
                ...
                $resultatrenvoye .= '<form action="' . route('Professionnels.declarationHeures', $user_id) . '" method="post">';
	            $resultatrenvoye .= csrf_field();
				$resultatrenvoye .= '<input type="hidden" name="mois" value="'.$mois.'">' ;
				$resultatrenvoye .= '<input type="hidden" name="annee" value="'.$annee.'">' ;
				$resultatrenvoye .= '<input type="hidden" name="user_id" value="'.$user_id.'">' ;
...
function Soumettre ( $mois , $annee , $user_id , $info )
{
    ...
    for ($i = 1; $i <= $maxdaymois; $i++) {
            $heure = array_key_exists($i, $info['Heures']) ? $info['Heures'][$i] : '';
    $conge = array_key_exists($i, $info['Conges']) ? $info['Conges'][$i] : 'FALSE';
    $maladie = array_key_exists($i, $info['Maladie']) ? $info['Maladie'][$i] : 'FALSE';
    $remarque = array_key_exists($i, $info['Remarque']) ? $info['Remarque'][$i] : '';
}
    $stockage .= ';' . $heure . ';' . $conge . ';' . $maladie . ';' . $remarque;
...
@php
$mois_etudie = 3;
$idemploye = request('idemploye') ? request('idemploye') : request('user_id');
...
if ($statut == "Sauvegarder") {
    Sauvegarde($mois_etudie, $an, $idemploye, request()->all());
    echo AfficheMois($mois_etudie, $an, $idemploye);
} elseif ($statut == "Soumettre") {
    Soumettre($mois_etudie, $an, $idemploye, request()->all());
    $valeurpdf = AfficheMois($mois_etudie, $an, $idemploye);
    echo AfficheMois_valide($mois_etudie, $an, $idemploye, $valeurpdf);
} elseif ($statut == "Reinitialiser") {
    Efface($mois_etudie, $an, $idemploye);
    echo AfficheMois($mois_etudie, $an, $idemploye);
} elseif ($statut == "Annule Demande") {
