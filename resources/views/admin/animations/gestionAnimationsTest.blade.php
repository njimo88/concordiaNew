...
<main id="main" class="main">
            ...
            <div class="table-container">
                <table id="table-animations-a-venir" class="table nowrap"  style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr style="background-color: {{ $animation->category->color }}; color: {{ $animation->category->text_color }}; border-color: transparent;" data-bs-toggle="modal" data-bs-target="#editAnimationModalTest" data-backdrop="static"
                                data-id="{{ $animation->id }}"
                                date-updater="{{ $animation->updated_by }}"
                                date-updated_at="{{ $animation->updated_at }}"
                                ...
                                data-end="{{ \Carbon\Carbon::parse($animation->animation_endtime)->format('d/m/Y à H\hi') }}"
                                ...
                                data-image="{{ $animation->image_path }}">
                                    <td>
                                            ...
                                            au {{ \Carbon\Carbon::parse($animation->animation_endtime)->format('d/m/Y à H\hi') }}
                                    </td>
                                    <td>
                                    ...
                                    <td>{{ $animation->participants->count() }}/{{ $animation->price }}</td>
        ...
        @if ($animationsPassees->isNotEmpty())
            ...
            <div class="table-container">
                <table id="table-animations-passees" class="table nowrap"  style="width:100%">
                        ...
                        <tbody>
                            @foreach ($animationsPassees as $animation)
                            <tr style="background-color: {{ $animation->category->color }}; color: {{ $animation->category->text_color }}; border-color: transparent;" data-bs-toggle="modal" data-bs-target="#editAnimationModal" data-backdrop="static"
                                data-id="{{ $animation->id }}"
                                date-updater="{{ $animation->updated_by }}"
                                date-updated_at="{{ $animation->updated_at }}"
                                data-title="{{ $animation->title }}"
    ...
    @include("admin.animations.modals.editAnimationTest")
</main>
@endsection
...
<script>
    ...
    $('tbody tr').on('click', function () {
        ...
        $('#modal-date').html(`Du <strong>${start}</strong> au <strong>${end}</strong>`);
