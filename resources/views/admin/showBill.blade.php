...
@section('content')
    <main class="main" id="main"
        style="background-image: url('{{ asset('/assets/images/background.png') }}'); min-height:100vh;">
        <!-- Modal -->
                    ...
                    <div class="modal-body">
                        <form action="{{ route('additional-charge.store') }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="bill_id" id="bill_id" required class="form-control"
        ...
        <div style="background-color: white;" class="container  justify-content-center">
            <div class="row ">
                            ...
                            <div class="row justify-content-end">
                                ...
                                <div class="col-md-3 col-sm-3 d-flex justify-content-end">
                                    <form method="post" action="{{ route('generatePDFfacture', $bill->id) }}"
                                        ...
                                        <button type="submit" class="custom-btn btn btn-primary">
                                            Fac <img src="{{ asset('assets/images/pdf-icon.png') }}" alt="PDF Icon"
                                ...
                                <div class="col-md-2 col-sm-4 d-flex justify-content-star">
                                    <form method="post" action="{{ route('generatePDFreduction_Fiscale', $bill->id) }}"
                                        ...
                                        @csrf
                                        <button type="submit" class="custom-btn btn btn-primary">
                                            Red Fisc <img src="{{ asset('assets/images/pdf-icon.png') }}" alt="PDF Icon"
                                                class="ml-2 icon-size">
                                        </button>
                                    </form>
                                </div>
                ...
                <style>
                    .custom-btn {
                        ...
                        width: 100%;
                        /* Full width to fill the parent container */
                        margin: 20px 0;
                ...
                @if (auth()->user()->roles->changer_status_facture && Route::currentRouteName() === 'facture.showBill')
                    ...
                    @else
                        <form>
                @endif
                <div style="background-color: @if ($bill->row_color == 'none') #00ff00 @else {{ $bill->row_color }} @endif"
                    class="mb-3 row d-flex justify-content-between">
                    <div class="col-md-5 p-4 col-12">
                        <div class="d-flex gap-2 align-items-center">
                            @if (!auth()->user()->roles->changer_status_facture || Route::currentRouteName() !== 'facture.showBill')
                                <select disabled class="border col-md-12 form-select @error('role') is-invalid @enderror"
                                    name="status" id="status" autocomplete="status" autofocus role="listbox">
                                    @foreach ($status as $s)
                                        @if ($s->id == $bill->status)
                                            <option value="{{ $s->id }}" selected role="option">
                                                {{ $s->status }}
                                            </option>
                                        @endif
                                    @endforeach
                            ...
                            @else
                                <select class="border col-md-12 form-select @error('role') is-invalid @enderror"
                                    name="status" id="status" autocomplete="status" autofocus role="listbox">
                                    @foreach ($status as $status)
                                        <option value="{{ $status->id }}"
                                            {{ $bill->status == $status->id ? 'selected' : '' }} role="option">
                                            {{ $status->status }}</option>
                                    @endforeach
                                </select>
                            @endif

                            @if (Auth::user()->roles->estAutoriserDeModifLevelWarning)
                                <span class="warning-icon" data-warning-level={{ $bill->level }}>
                                </span>
                            @endif
                        </div>
                    </div>
            ...
            <div class="row border border-dark m-1 mb-4 rounded">
                <div class="col-md-4 col-12 p-3">
                    ...
                    <span style="font-weight:bold">{{ $bill->lastname }} {{ $bill->name }} (ID n°{{ $bill->user_id }})
                    </span> <br>
                    <a style="color:rgb(4, 0, 255)" class="a" href="mailto:{{ $bill->email }}">{{ $bill->email }}
                        <br>
                        <a style="color:rgb(4, 0, 255)" class="a"
                            href="tel:{{ $bill->phone }}">{{ $bill->phone }}</a><br>
                        {{ \Carbon\Carbon::parse($bill->birthdate)->age }} ans
                        <br>

                        @if (Auth::user()->roles->estAutoriserDeModifLevelWarning)
                            <strong>Niveau de warning : </strong>&nbsp;
                            <div class="d-flex">
                                <select id="warning-level" class="form-select" name="warning-level"
                                    {{ Auth::user()->roles->estAutoriserDeModifLevelWarning ? '' : 'disabled' }}
                                    value={{ $bill->level }}>
                                    <option value="0">Aucun warning</option>
                                    <option value="1" @if ($bill->level == 1) selected @endif>1
                                    </option>
                                    <option value="2" @if ($bill->level == 2) selected @endif>2
                                    </option>
                                </select>
                                <span class="warning-icon" data-warning-level={{ $bill->level }}>
                                </span>
                ...
                <div class="col-md-4 col-12 p-3">
                    ...
                    <select name="payment_method" id="payment_method">
                        @foreach ($paymentMethods as $method)
                            <option value="{{ $method->id }}"
                                {{ $bill->payment_method == $method->id ? 'selected' : '' }}>
                    ...
                    <span style="font-weight:bold">Frais </span>: {{ number_format($bill->total_charges, 2, ',', ' ') }}
                    €<br>

                    <div class="form-group">
                        <label for="payment_total_amount"><strong style="color: black">Coût TTC:</strong></label>
                        <input style="width: 30%" type="text" id="payment_total_amount" class="form-control"
                            name="payment_total_amount"
                            value="{{ number_format($bill->payment_total_amount, 2, ',', ' ') }} €">
                    </div>
                    <br>
                </div>
                <div class="col-md-4 col-12 p-3">
                    ...
                    <span style="font-weight:bold">
                        ...
                        @else
                            {{ number_format($bill->payment_total_amount - $bill->amount_paid, 2, ',', ' ') }} €
                    </span>
                    @endif
                    <br><br><br>
                    @if (auth()->user()->roles->paiement_immediat && Route::currentRouteName() === 'facture.showBill')
                        <a href="{{ route('paiement_immediat', $bill->id) }}"
                            class="my-custom-btn btn btn-primary my-4 p-2">Paiement Immédiat <img style="width: 30px"
                                src="{{ asset('assets/images/fds.png') }}" alt=""></a>
                    @endif
                    <!-- Button trigger modal -->
                    <button type="button" class="btn-danger" data-bs-toggle="modal"
                        data-bs-target="#additionalChargeModal">
            ...
            <div class="row border border-dark mx-2">
                <div class="col-12 d-flex justify-content-center border-bottom -border-dark bg-secondary">
                    <h3 class="my-2">Détail Facture</h3>
                    <hr>
                </div>
            </div>
            @if (count($shop) > 0)
                        ...
                        @foreach ($shop as $shop)
                            ...
                            @else
                                <tr class="responsive-table-row">
                                        ...
                                        <div class="td">
                                            ...
                                            @if (!$isCurrentSeason)
                                                <input type="text" class="form-control mt-3"
                                                    value="{{ $shop->designation }}" disabled>
                                            @else
                                                @if (!auth()->user()->roles->changer_designation_facture)
                                                    <select disabled name="designation"
                                                        class="border form-select mt-3 designation-select" id="status"
                                                        ...
                                                        @foreach ($designation as $title)
                                                            @if ($title == $shop->designation)
                                                                <option value="{{ $title }}" role="option"
                                                                    selected>
                                                                    {{ $title }}@if (!empty($shop->declinaison_libelle))
                                                ...
                                                @else
                                                    <select name="designation"
                                                        ...
                                                        @foreach ($designation as $article)
                                                            <option value="{{ $article->id_shop_article }}"
                                                                data-declinaison-id="{{ optional($article->declinaisons->first())->id }}"
                                                                role="option"
                                                                @if ($article->id_shop_article == $shop->id_shop_article) selected @endif>
                                                                {{ $article->title }}@if (!empty($article->declinaisons->first()->libelle))
                                                ...
                                                @if (auth()->user()->roles->changer_designation_facture)
                                                    ...
                                                    <button type="button"
                                                        class="btn btn-sm btn-warning mt-3 change-designation-button"
                                    ...
                                    <td class="d-none d-md-table-cell">{{ number_format($shop->sub_total, 2, ',', ' ') }}
                                        €</td>
                                    <td>
            ...
            @if (auth()->user()->roles->changer_designation_facture && Route::currentRouteName() === 'facture.showBill')
                <div class="d-flex justify-content-end m-3">
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                        data-bs-target="#AjouteProduit">
                        <i class="fa-solid fa-plus"></i> &nbsp; Ajouter un produit
                </div>
            @endif
            <div class="modal fade" id="AjouteProduit" tabindex="-1" aria-labelledby="AjouteProduitLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
            ...
            <div class="row border border-dark my-5 mx-2">
                <div class="col-12 d-flex justify-content-center border-bottom -border-dark bg-secondary">
                    <h3 class="my-2">Historique des modification :</h3>
                    <hr>
                </div>
                            ...
                            @if ($message->state == 'Public' || Route::currentRouteName() === 'facture.showBill')
                                <div>
                                    @if ($message->state == 'Privé')
                                        <u><b><span style="color:red;">(Privé)</span></b> {{ $message->lastname }}
                                            {{ $message->name }} <time
                                                datetime="{{ $message->date }}">{{ $formattedDate }}</time></u><br>
                                    @else
                                        <u>{{ $message->lastname }} {{ $message->name }} <time
                                                datetime="{{ $message->date }}">{{ $formattedDate }}</time></u><br>
                                    @endif

                                    @if ($message->somme_payé <= 0 && $message->somme_payé != null)
                                        <b>Somme payée : </b><span style="font-weight : bold"
                                            class="text-danger">{{ $message->somme_payé * -1 }} €</span><br>
                                    @elseif ($message->somme_payé > 0)
                                        <b>Somme remboursée : </b><span style="font-weight : bold"
                                            class="text-success">{{ $message->somme_payé }} €</span><br>
                                    @endif
                                </div>
                                ...
                                <div class="col-1 my-2 d-flex justify-content-end">
                                    <button
                                        style="display: flex ; align-self:flex-end"class="btn  btn-outline-danger delete-button"
                                        data-id="{{ $message->id_shop_message }}">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.change-designation-button').click(function() {
                ...
                var declinaisonId = $(this).closest('tr').find('.designation-select option:selected').data(
                    'declinaison-id');
        ...
        document.getElementById("warning-level").addEventListener("change", function() {
            updateFamilyWarningLevel({
                family_id: @json($bill->family_id),
                level: this.value
            });
        });

        function updateFamilyWarningLevel(data) {
            fetch('/update-warning-level', {
                    method: 'POST',
                    headers: {
        ...
        $.ajax({
            ...
            success: function(products) {
                products.forEach(function(product) {
                    var option = $('<option></option>').val(product.id_shop_article).text(product
                        .title);
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(familyMembers) {
                    familyMembers.forEach(function(member) {
                        var option = $('<option></option>').val(member.user_id).text(member
            ...
            $.ajax({
                ...
                success: function(products) {
                    products.forEach(function(product) {
                        var option = $('<option></option>').val(product.id_shop_article).text(
                            product.title);
            ...
            $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(familyMembers) {
                        familyMembers.forEach(function(member) {
                            var option = $('<option></option>').val(member.user_id)
                                .text(member.lastname + ' ' + member.name);
                            familySelect.append(option);
                        });
                    },
                    error: function(error) {
