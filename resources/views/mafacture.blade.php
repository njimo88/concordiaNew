...
@section('content')
    ...
    @endif
    <section class="container-fluid ps-4 py-2"
        style="background-color: @if ($bill->row_color == 'none' || empty($bill->row_color)) #00ff00 @else {{ $bill->row_color }} @endif">
        <div class="container-xxl">
            <div class="row justify-content-between align-items-center">
                <div class="col-12 col-xl-5">
                    <p class="fw-bold text-white mb-0 link-white">
                        <i class="far fa-lg fa-newspaper me-2 me-md-3 text-white"></i>
                        <a href="#" rel="category tag">Facture n°{{ $bill->id }}</a>&nbsp;&nbsp;/
                        &nbsp;&nbsp;{{ $bill->bill_status }}
                    </p>
                </div>
                ...
                @if ($bill->status >= 70)
                    <div class="col-auto">
                        <form method="post" action="{{ route('generatePDFfacture', $bill->id) }}" class="d-inline">
                            @csrf
                            <button type="submit" class="my-custom-btn btn btn-primary my-2">Facture <img
                                    src="{{ asset('assets\images\pdf-icon.png') }}" alt=""></button>
                        </form>
                        <form method="post" action="{{ route('generatePDFreduction_Fiscale', $bill->id) }}"
                            class="d-inline">
                            @csrf
                            <button type="submit" class="my-custom-btn btn btn-primary my-2">Réduction Fiscale <img
                                    src="{{ asset('assets\images\pdf-icon.png') }}" alt=""></button>
                        </form>
                    </div>
                @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 col-12 p-0 cardd">
                                ...
                                <div class="cardd-body">
                                    <p><strong>{{ $bill->lastname }} {{ $bill->name }} (ID
                                            n°{{ $bill->user_id }})</strong></p>
                                    <p><i class="fa fa-envelope"></i> <a
                                            href="mailto:{{ $bill->email }}">{{ $bill->email }}</a></p>
                                    <p><i class="fa fa-phone"></i> <a
                                            href="tel:{{ $bill->phone }}">{{ $bill->phone }}</a></p>
                            ...
                            <div class="col-md-4 col-12 p-0 cardd">
                                ...
                                <div class="cardd-body">
                                    ...
                                    <span style="font-weight:bold">Frais </span>:
                                    {{ number_format($bill->total_charges, 2, ',', ' ') }} €<br>
                                    <span style="font-weight:bold">Coût TTC </span>:
                                    {{ number_format($bill->payment_total_amount, 2, ',', ' ') }} €<br>
                                </div>
                            </div>
                            <!-- Détail paiement -->
                                ...
                                <div class="cardd-body">
                                    ...
                                    @if (auth()->user()->roles->paiement_immediat && Route::currentRouteName() === 'facture.showBill')
                                        <a href="{{ route('paiement_immediat', $bill->id) }}"
                                            class="my-custom-btn btn btn-primary my-4 p-2">Paiement Immédiat <img
                                                style="width: 30px" src="{{ asset('assets/images/fds.png') }}"
                                                alt=""></a>
                                    @endif
                                    @if ($additionalCharge)
                                        <a href="{{ route('payment_formFrais', ['nombre_virment' => 1, 'total' => $additionalCharge->amount, 'bill_id' => $bill->id]) }}"
                                            class="my-custom-btn btn btn-primary my-4 p-2">Paiement partiel
                                            ({{ number_format($additionalCharge->amount, 2, ',', ' ') }} €) <img
                                                style="width: 30px" src="{{ asset('assets/images/fds.png') }}"
                                                alt=""></a>
                                    @endif
                                </div>
                                                ...
                                                <td style="width: 600px;">
                                                    <div class="row">
                                                        <div class="col-md-2 col-12">
                                                            <img style="height: 70px" src="{{ $item->image }}"
                                                                alt="">
                                                        </div>
                                                        <div class="col-md-6 col-12">
                                                            @if ($item->id_shop_article)
                                                                <!-- Lien cliquable si l'ID est présent -->
                                                                <a href="{{ route('singleProduct', ['id' => $item->id_shop_article]) }}"
