                            ...
                            <div class="img-container">
                                @foreach ($teachers as $teacher)
                                    <div class="teacher-card">
                                        <img class="teacher-image" src="{{ $teacher->image }}"
                            ...
                            <div class="locations-section">
                                ...
                                <ul class="location-list">
                                    @foreach ($locations as $location)
                                        <li class="mt-2">
                                            <a style="color: #182881" href="{{ $location['map'] }}"
                                                target="_blank">{{ $location['name'] }}</a>
                                            <br>
                                            <a style="color: #182881" href="{{ $location['map'] }}" target="_blank"
                                                class="text-xs">{{ $location['address'] }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                                    ...
                                    @else
                                        @if ($articl->stock_actuel > 0)
                                            <h4 class="card-title">
                                                ...
                                                @elseif($articl->type_article == 1 && $coursVente == 0 && $userAchetéType0 && $coursVenteMember == 1)
                                                    <select onchange="updatePriceToDisplay()" class="select-form" name="buyers"
                                                        id="buyers">
                                                    ...
                                                    <button data-shop-id="{{ $articl->id_shop_article }}"
                                                        class="btn commanderModal">Inscrire</button>
                                                @elseif($articl->type_article == 1 && $coursVente == 0 && !$userAchetéType0 && $coursVenteMember == 1)
                                                    <div class="desc-reduc-ribbon fa-check-cross">
                                                        <i class="fa-solid fa-hourglass"></i> En vente prochainement
                                                    </div>
                                            ...
                                            @if (count($selectedUsers) > 0)
                                                @if (count($declinaisons) > 0)
                                                    <select class="select-form" name="declinaisons" id="declinaisons">
                                                        ...
                                                        @foreach ($declinaisons as $declinaison)
                                                            @if ($declinaison->stock_actuel_d > 0)
                                                                <option value="{{ $declinaison->id }}">
                                                                    {{ $declinaison->libelle }}</option>
                                                                </option>
                                                            @endif
                                                        @endforeach
                                                    </select>
                                                ...
                                                @if ($articl->type_article == 2)
                                                    <input type="number" class="select-form" name="qte" id="qte"
                                                        min="1" value="1">
                                                @endif

                                                @if ($articl->type_article == 3)
                                                    <form
                                                        action='{{ route('choisir_place', ['id' => $articl->id_shop_article]) }}'
                                                        method="POST">
                                                        @csrf
                                                ...
                                                @elseif(!($articl->type_article == 1 && $coursVente == 0))
                                                    <button data-shop-id="{{ $articl->id_shop_article }}"
                                                        class="btn commanderModal">Commander</button>
                                                @endif
                                            @elseif ($coursVente != 0)
                                                <p class="info-message">Votre famille ne correspond pas à cet article.</p>
                                            @endif
                                        @else
                                            <p class="info-message">Désolé, cet article est actuellement en rupture de stock.
                                            </p>
                                        @endif
                                    @endguest
                                </div>
                ...
                <div class="product-div-right">
                    ...
                    <div class="price-and-new-badge">
                        ...
                        @if ($articl->nouveaute == 1)
                            <div class="new-badge"><img src="{{ asset('assets/images/Nouveau.png') }}" alt="Nouveau">
                            </div>
                        @endif
                    </div>
            ...
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if ({{ $articl->totalprice }} != reducedPrice) {
                        ...
                        priceHTML += ({{ $articl->totalprice }}).toLocaleString('fr-FR', {
                            style: 'currency',
                            currency: 'EUR'
                        });
                        priceHTML += " </span>";
                        ...
                        priceHTML += priceHTML += '<span style="color: red; font-size: x-large; font-weight: bold;">' +
                            ({{ $articl->totalprice }}).toLocaleString('fr-FR', {
                                style: 'currency',
                                currency: 'EUR'
                            });
                        priceHTML += " </span>";
