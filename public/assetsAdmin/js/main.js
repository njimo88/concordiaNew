...
$(function () {
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return (
                        '<td data-user-id="' +
                        row.user_id +
                        '" class="bill a" style="font-weight: bold; font-family:Arial, Helvetica, sans-serif">' +
                        data +
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    buttons +=
                        '<span class="d-inline-block mx-2" tabindex="0" data-bs-toggle="tooltip" title="Factures famille">' +
                        '<button data-family-id="' +
                        row.family_id +
                        '" type="button" class="btn btn-sm btn-primary familybill">' +
    ...
    $('#customSearch').on('keyup', function (e) {
        if (e.key === 'Enter') {
            ...
            table.ajax
                .url('/admin/paiement/facture/data?searchTerm=' + searchTerm)
                .load()
        }
    })
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data === 'Aucune facture trouvée.') {
                $('#familyBillsContainer').html(
                    '<div style="display: block; color: black; margin: auto; text-align: center; padding: 10px;">Aucune donnée disponible</div>',
                )
            } else {
...
$(document).ready(function () {
            ...
            success: function (data) {
                ...
                if (data.length == 0) {
                    $('#linked-users-list').append(
                        '<li class="list-group-item">Aucun utilisateur lié</li>',
                    )
                } else {
                    data.forEach((user) => {
                        $('#linked-users-list').append(
    ...
    $(document).on(
        'change',
        '#user-search-results input[type="checkbox"]',
        function () {
            ...
            if ($(this).is(':checked')) {
                ...
                $(
                    '#linked-users-list li[data-user-id="' + userId + '"]',
                ).remove()
            }
        },
    )
                ...
                success: function (data) {
                    ...
                    if (data.length == 0) {
                        $('#user-search-results').append(
                            '<div class="p-2">Aucun utilisateur trouvé</div>',
                        )
                    } else {
                        data.forEach((user) => {
                            $('#user-search-results').append(
    ...
    $(document).on('click', '.remove-user', function () {
        ...
        $.ajax({
            ...
            error: function (error) {
                console.error(
                    "Erreur lors de la suppression de l'utilisateur lié",
                    error,
                )
            },
        })
...
$(document).ready(function () {
    ...
    $('#confirmPreparation').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('.btn-prepared[data-liaison-id="' + liaisonId + '"]')
                    .closest('.col-12.col-md-6.col-lg-4.mb-3')
    ...
    $('.nonConcerne').on('click', function () {
        ...
        var isConfirmed = confirm(
            "Êtes-vous sûr que le produit n'est pas concerné ?",
        )
...
function changeTab(tabId) {
    ...
    document
        .querySelector('.tab[onclick="changeTab(\'' + tabId + '\')"]')
    ...
    $('#confirmDistribution').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('.btn-distribute[data-liaison-id="' + liaisonId + '"]')
                    .closest('.col-12.col-md-6.col-lg-4.mb-3')
                    .fadeOut(500, function () {
                        $(this).remove()
                    })
...
$('#submitReturn').on('click', function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('.btn-return[data-liaison-id="' + liaisonId + '"]')
                .closest('.col-12.col-md-6.col-lg-4.mb-3')
                .fadeOut(500, function () {
                    $(this).remove()
                })
