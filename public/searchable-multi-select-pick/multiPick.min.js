$.fn.extend({
    multiPick: function (config) {
        ...
        let options = $(`#${id} option`),
            itens = $.map(options, function (option) {
                return option
            })
        var itemFormat = ''
        ...
        !0 === settings.slim && (slim = 'slim'),
            ...
            $(document).ready(function () {
                $(`#${id}`).click(function (e) {
                    e.stopPropagation(),
                        $(`.mutiple-select:not(#${id}) .itens-list`).slideUp(
                            'fast',
                        ),
                        ...
                        if ($(`#${id}`).find('#form-search').val().length > 0)
                            for (var i = 0; i < options.length; i++) {
                                ...
                                var nome = $(option).prop('id'),
                                    expressao
                                new RegExp(this.value, 'i').test(nome)
                                    ? 'search' !== nome &&
