$.fn.extend({
    multiPick: function (config) {
        ...
        var slim = ''
        if (settings.slim === true) slim = 'slim'

        let stringSetting = JSON.stringify(settings).replaceAll('"', `'`)

        $(item).parent()
            .prepend(`<div class="mutiple-select ${slim}" id="${id}" data-settings="${stringSetting}">
        ...
        $(document).ready(function () {
            ...
            $(`#${id}`)
                ...
                .click(function (e) {
                    ...
                    let itens = $(`#${id}`).find(
                        `.main-content .selected-itens .item`,
                    )

                    let selectedContentWidth = $(`#${id}`)
                        .find(`.main-content .selected-itens`)
                        .width()
                    ...
                    for (var i = 0; i < itens.length; i++) {
                        ...
                        if (value == $(item).data('value')) same = true
                    }
                    ...
                    if (
                        $(this).prop('id') !== 'search' &&
                        itens.length < settings.limit &&
                        same === false
                    ) {
                        $(`#${id}`).find(`.selected-itens`)
                            .append(`<div class="item" data-value="${value}">
                                                            ${imageblock}
                                                            ${text}
            ...
            $(`#form-search`).on('keyup', function () {
                ...
                if ($(`#${id}`).find(`#form-search`).val().length > 0) {
                    ...
                    for (var i = 0; i < options.length; i++) {
                        let option = options[i]
                        if (nome !== 'search') $(option).css('display', 'flex')
                    }
                }
            })
        })
        ...
        function placeHide() {
            if (
                $(`#${id}`).find(`.main-content .selected-itens .item`)
                    .length === 0
            ) {
