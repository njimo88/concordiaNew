                ...
                if (config.scaleShowLabels) {
                    ...
                    if (config.scaleShowLabelBackdrop) {
                        ...
                        ctx.rect(
                            ...
                            Math.round(
                                textWidth + config.scaleBackdropPaddingX * 2,
                            ), //Width
                            Math.round(
                                config.scaleFontSize +
                                    config.scaleBackdropPaddingY * 2,
                            ), //Height
                        )
                        ctx.fill()
                    }
    ...
    var Radar = function (data, config, ctx) {
        ...
        function drawScale() {
                ...
                if (config.scaleShowLabels) {
                    ...
                    if (config.scaleShowLabelBackdrop) {
                        var textWidth = ctx.measureText(
                            calculatedScale.labels[i],
                        ).width
                        ctx.fillStyle = config.scaleBackdropColor
                        ctx.beginPath()
                        ctx.rect(
                            Math.round(
                                -textWidth / 2 - config.scaleBackdropPaddingX,
                            ), //X
                            Math.round(
                                -scaleHop * (i + 1) -
                                    config.scaleFontSize * 0.5 -
            ...
            for (var k = 0; k < data.labels.length; k++) {
                ...
                var opposite =
                    Math.sin(rotationDegree * k) *
                    (maxSize + config.pointLabelFontSize)
                var adjacent =
                    Math.cos(rotationDegree * k) *
                    (maxSize + config.pointLabelFontSize)
        ...
        function calculateXAxisSize() {
            ...
            if (config.scaleShowLabels) {
                ...
                for (var i = 0; i < calculatedScale.labels.length; i++) {
                    var measuredText = ctx.measureText(
                        calculatedScale.labels[i],
                    ).width
                    longestText =
