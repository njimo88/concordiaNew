	...
	class TCPDF {
		...
		public function unichr($c) {
			if (!$this->isunicode) {
				...
				// two bytes
				return chr(0xC0 | intval($c) >> 6).chr(0x80 | intval($c) & 0x3F);
			} elseif ($c <= 0xFFFF) {
				// three bytes
				return chr(0xE0 | intval($c) >> 12).chr(0x80 | intval($c) >> 6 & 0x3F).chr(0x80 | intval($c) & 0x3F);
			} elseif ($c <= 0x10FFFF) {
				// four bytes
				return chr(0xF0 | intval($c) >> 18).chr(0x80 | intval($c) >> 12 & 0x3F).chr(0x80 | intval($c) >> 6 & 0x3F).chr(0x80 | intval($c) & 0x3F);
			} else {
				return '';
			}
		}
