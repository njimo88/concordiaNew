            ...
            elseif ($data !== null) {
                ...
                if (method_exists(Format::class, 'to_'.$this->response->format)) {
                    ...
                    if (is_array($data) || is_object($data)) {
                        $data = chriskacerguis\RestServer\Format::factory($data)->{'to_json'}();
                    }
        ...
        } elseif ( $this->input->server('HTTP_AUTHENTICATION') || $this->input->server('REDIRECT_REMOTE_USER') )
{
    ...
    protected function _prepare_digest_auth()
    {
        ...
        $valid_response = md5($this->_check_login($digest['username'], TRUE).':'.$digest['nonce'].':'.$digest['nc'].':'.$digest['cnonce'].':'.$digest['qop'].':'.$md5);
