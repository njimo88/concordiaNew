...
export default class FilterMultiSelect {
    ...
    private static createOptions(
        fms: FilterMultiSelect,
        name: string,
        htmlOptions: Array<HTMLOptionElement>,
        jsOptions: Array<
            [
                label: string,
                value: string,
    ...
    private static UnrestrictedSelectAllOption = class
        extends FilterMultiSelect.SingleOption
        implements SelectAllOption
    {
    ...
    private static RestrictedSelectAllOption = class
        implements SelectAllOption
    {
    ...
    constructor(selectTarget: JQuery<HTMLElement>, args: Args) {
        ...
        this.name = name
        let array: Array<HTMLOptionElement> = selectTarget
            .find('option')
            .toArray()
    ...
    private updateDropdownList(): void {
        ...
        if (this.caseSensitive) {
            ...
            this.options.forEach((o: Option, i: number) => {
                if (
                    o.getLabel().toLowerCase().indexOf(text.toLowerCase()) !==
                    -1
                ) {
                    //magic number
                    o.show()
        ...
        do {
            ...
            (i == -1 &&
                (this.selectAllOption.isDisabled() ||
                    this.selectAllOption.isHidden())) || //magic number
            (i >= 0 &&
    ...
    public getSelectedOptionsAsJson(includeDisabled = true): string {
        const data: any = {}
        let a: Array<string> = this.getSelectedOptions(includeDisabled).map(
            (o) => o.getValue(),
        )
        data[this.getName()] = a
    ...
    private dispatchEvent(
        ...
        let event: CustomEvent = FilterMultiSelect.createEvent(
            eventType,
            this.getName(),
            value,
            label,
